...
router.get('/:id', auth, adminOnly, async (req, res) => {
  try {
    const alumniRepository = AppDataSource.getRepository(Alumni);
    const alumni = await alumniRepository.findOne({ where: { id: req.params.id } });
...
router.post('/', auth, adminOnly, upload.single('profileImage'), async (req, res) => {
  try {
    ...
    const { fullName, email, graduationYear, currentOccupation } = req.body;
    if (!fullName || !email || !graduationYear || !currentOccupation) {
...
router.put('/:id', auth, adminOnly, upload.single('profileImage'), async (req, res) => {
  try {
    ...
    if (req.file) {
      updateData.profileImage = `/uploads/alumni/${req.file.filename}`;
    }
    
    await alumniRepository.update(alumniId, updateData);
    const updatedAlumni = await alumniRepository.findOne({ where: { id: alumniId } });
...
router.patch('/:id/approve', auth, adminOnly, async (req, res) => {
  try {
    ...
    const updatedAlumni = await alumniRepository.findOne({ where: { id: req.params.id } });
    ...
    res.json({ message: 'Alumni profile approved and made public', alumni: updatedAlumni });
  } catch (error) {
